 ...
 * CVS: $Id: ee_thendin.c,v 1.1 2008/04/23 11:36:01 francesco Exp $
 */
...
void EE_thread_end_instance(void)
{
  ...
  if (EE_th_nact[EE_exec] > 0) {
    if(EE_iris_updatecapacity(EE_exec, tmp_time) == InsertRCGQueue){
  ...
  if ((EE_stk_queryfirst() != EE_NIL && EE_rq_queryfirst() == EE_NIL) ||  // note that this test work also for the main task!
