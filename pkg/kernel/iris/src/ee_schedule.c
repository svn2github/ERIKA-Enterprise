 ...
 * CVS: $Id: ee_schedule.c,v 1.1 2008/04/23 11:36:01 francesco Exp $
 */
...
void EE_iris_sys_scheduler(void)
{
  ...
  if (EE_exec == EE_NIL ||	/* main task! */
    ...
    if (old_exec != EE_NIL) {
      ...
      if(EE_iris_updatecapacity(old_exec, tmp_time) == InsertRCGQueue && !EE_th_lockedcounter[old_exec]){
        EE_rcg_insert(old_exec);
        EE_th_status[old_exec] = EE_RECHARGING;
