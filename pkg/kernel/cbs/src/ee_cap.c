 ...
 * CVS: $Id: ee_cap.c,v 1.2 2007/06/14 10:27:12 pj Exp $
 */
...
void EE_cbs_updatecapacity(EE_TID t, EE_TIME tmp_time)
{
  ...
  if (!EE_th_active[t] || (EE_STIME)(EE_th_absdline[t] - tmp_time) < 0) {
    ...
    while (EE_th_budget_avail[t] < EE_TIMER_MINCAPACITY) {
      /* if adding the relative deadline do not wraps around */
      if ((EE_STIME)(EE_th_absdline[t] + EE_th_period[t] - tmp_time)>0) {
	/* I hope that only 1-2 iterations are enough */
	EE_th_absdline[t] += EE_th_period[t];
	EE_th_budget_avail[t] += EE_th_budget[t];
      }
      else {
	EE_th_absdline[t] = tmp_time+EE_TIMER_MAXFUTUREVALUE;
	EE_th_budget_avail[t] = EE_TIMER_MINCAPACITY;
	return;
      }
    }
  }
