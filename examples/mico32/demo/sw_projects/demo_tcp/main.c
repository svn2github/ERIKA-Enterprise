/*
	...
	Description: 	LWIP TCP test (with ENC28J60, see oil files).
...
err_t tcp_connect_callback(void *arg, struct tcp_pcb *pcb, err_t err)
{
	...
	mssg->pos  = 0;
     ...
     callbacks. */
  	tcp_arg(pcb, mssg);						/* Set the argument that will be passed 
  											 * to all the callbacks for that connection */
