...
TASK(Task1)
{
    EE_UINT8 uart_byte;
 	...
 	if( EE_uart_read_byte(&uart_byte) < 0 )
    {
//    	/* Turn on ERROR led */
//    	if (! byte_read)
//	    	write_led(LED2);
...
//    	write_led(NONE_LED);
#if 0
    	if( EE_uart_write_byte(uart_byte) < 0 )
    	{
    		/* Turn on ERROR led */
    		write_led(LED3);
    	}
...
int main(void)
{
	...
	EE_uart_set_mode(EE_UART_POLLING);	// non blocking mode
	#else
	EE_uart_set_mode(EE_UART_POLLING | EE_UART_RXTX_BLOCK); // blocking mode 
	#endif
	
	EE_uart_set_rx_callback(rx_cbk);
